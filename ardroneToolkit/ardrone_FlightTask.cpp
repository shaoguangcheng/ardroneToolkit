...
int FlightTask::keyEvent(InputEvent *event)
{
    if( event != NULL ){
        if( event->type == OSA_KEY_PRESS ) {
            ...
            //attituds control
            if( event->code == OSA_VK_a){ yaw = -10000.0;}
            if( event->code == OSA_VK_d){ yaw = 10000.0;}
            if( event->code == OSA_VK_LEFT){roll = -10000.0;}
            if( event->code == OSA_VK_RIGHT){roll = 10000.0;}
            if( event->code == OSA_VK_UP){pitch = -10000.0;}
            if( event->code == OSA_VK_DOWN){pitch = 10000.0;}
            if( event->code == OSA_VK_s){ height = 10000.0;}
            if( event->code == OSA_VK_w){ height = -10000.0;}
...
FlightTask_VanishPoint::FlightTask_VanishPoint():FlightTask()
{
    ...
    initYaw   = g_navData->psi;
    initRoll  = g_navData->theta;
    initPitch = g_navData->phi;
}
...
//            hx = (vp.x*dx-vp.y*dy+p_left.x*dy*dy/dx+p_left.y*dy)/(dy*dy/dx + dx);
 ...
 //         yaw = theta * 100;
            ...
            yaw = 5*(vp.x - center_x);
#else
            yaw  = (g_navData->psi - initYaw);
            roll = g_navData->theta -initRoll;
            dbg_p("yaw=%f (%f)", yaw, initYaw/1000.0);
            dbg_p("rol=%f (%f)", roll, initRoll/1000.0);
#endif
        }
        else
        {
            pitch = roll = yaw = height = 0;
            yaw  = 0.05*(g_navData->psi - initYaw);
            roll = 0.05*(g_navData->theta -initRoll);
            dbg_p("yaw=%f (%f)", yaw, initYaw/1000.0);
            dbg_p("rol=%f (%f)", roll, initRoll/1000.0);
        }
    }
...
{
    Mat img_gray,img_equ_hist,img_gaussi,img_canny,img_thresh;
    ...
    Canny(img_thresh,img_canny,40,80);
...
//        circle(image,Point(l[2],l[3]),2,Scalar(0,0,255));
    ...
    if(line_num <= 1) { vp = Point(0,0);return -1;};
    ...
    if(fabs(atan(k0)*180/PI-atan(k1)*180/PI)<20){vp = Point(0,0);return -1;}
